...
- Everything is in the book except one special tab. A session's Orca tab id and the bot's Orca project id live in the book with the session. The only exception is Bot Father's ops tab for whole-fleet maintenance: nothing that happens in it is tracked (no harness session id, no history, not seen by grooming). [decided] Like every tab it is found by its id, never by its title. [decided] That one tab id is kept as a single entry in Bot Father's book file, marked as not a session. [proposed, waiting for the owner]
