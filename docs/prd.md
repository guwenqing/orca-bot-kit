...
- Two developer sessions work in parallel in separate clones: one on the product code, one on the rules and skills; they share the reviewer. The coordinator session is not a relay: developers ask the reviewer directly (a line typed into its tab) and read the verdict on the PR. The coordinator hands out issues, answers questions, does the architecture review in the final round for the rules and skills, and calls for a second review only when a case is out of the ordinary. [decided]
