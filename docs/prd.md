...
- Two developer sessions work in parallel in separate clones: one on the product code, one on the rules and skills; they share the reviewer. The coordinator session is not a relay: developers ask the reviewer directly (a line typed into its tab), and the reviewer returns to whoever asked (the PR comment plus one line into the requester's tab). The coordinator hears from a developer only when it needs help, when something is out of the ordinary, or when it is done and needs the next issue. The coordinator hands out issues, answers questions, and steps in as the architect only when a case is out of the ordinary; there is no routine architecture review. [decided]
