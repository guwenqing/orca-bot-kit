...
- For now: one developer session (Claude, Opus 5, high effort) works one issue at a time. A separate reviewer session (Codex, Astra, high effort) in its own clone reviews each PR, only comments, and gives the verdict before merge. The design session stays outside as coordinator: it hands out the issues, routes the review, merges after an approval, clears the developer session between issues, decides most questions and takes only real owner decisions to the owner. [decided, temporary]
