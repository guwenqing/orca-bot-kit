...
- A prompt given as a positional argument (`claude [options] '<prompt>'`) starts an interactive session and is run as its first message, after the folder-trust question is answered. **verified** (live)
...
- A prompt given as a positional argument (`codex [options] '<prompt>'`) is run as the session's first message, after the trust question is answered. **verified** (live)
