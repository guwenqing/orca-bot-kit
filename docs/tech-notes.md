...
10. That `claude --resume <id> -n <other name>` gives the conversation the new name, so a session the kit named `<bot>.<session>` before #286 takes its new address on its next resume (section 2, #286).
