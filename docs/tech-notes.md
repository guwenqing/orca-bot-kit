...
- **`total_token_usage` is cumulative, and summing it is wrong.** It is the running total for the conversation, so adding it up across the events double counts: one conversation that used 80,565 tokens came to 200,901 when the totals were summed. Sum `last_token_usage` instead, or take the final `total_token_usage`; on the same conversation both gave 80,565 exactly. **verified** (2026-09-21, 0.155.1)
