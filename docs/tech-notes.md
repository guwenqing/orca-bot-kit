...
- **A skill added while a session runs is picked up only if the skills directory existed when the session started — on both harnesses.** Proved live on 2026-09-22 (Claude Code 2.1.280, Codex 0.155.1), for issue #157, adding a skill through `obk skills add` and `skills build` to sessions already running:

  | | the skills directory existed at start | it did not |
  |---|---|---|
  | Claude Code | the Skill tool loaded it: `Successfully loaded skill` | the Skill tool refused it: `Unknown skill: <name>` |
  | Codex | asked without naming the skill, it chose the new one from its description | asked the same way, it answered that it did not know; named, it found `SKILL.md` by looking, which is not the skill being registered |

  So "skill directories are watched" (Claude Code's docs, section 2) and "detects skill changes automatically" (Codex's docs, above) both hold only for a directory that was there when the session began; neither harness starts watching one created afterwards. A bot that starts with no skills has neither directory until it is given its first, and then cannot use it without a restart. **verified** (live, both harnesses)
