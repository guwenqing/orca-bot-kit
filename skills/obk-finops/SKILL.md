...
visible symptom. A worker session compacted more than once is a signal to look
at what the re-reading actually costs and what it has lost, not a verdict.
Recommend a written hand-over and a fresh start only on that evidence and only
with the user's say, and explain a decision to keep the session going as well.
A long-lived persona session (Bot Father, an inbox) is meant to run long, and is
judged on what the re-reading costs and whether it has lost what it knew.
