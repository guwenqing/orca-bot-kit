...
Where the reasons are the point, why it is shaped this way or where a number
came from, that is its own kind of digging. It is worth doing properly rather
than guessing at plausibly.
...
the subject would say it out loud. Use its ordinary name if it has one.
...
Give the smallest complete answer first, a sentence or two rather than a dense
...
find out what they know; read that from the conversation. Do not ask them to
say it back to prove it landed. Someone who asks to be tested on it is
...
Draw when a picture beats words. For anything with three or more moving parts,
do not draw one diagram with all of them. Draw a short series, each one
...
meaning is a diagram. Something spatial is a sketch, roughly drawn with a few
short labels: a layout, an overlap, a before and after.
