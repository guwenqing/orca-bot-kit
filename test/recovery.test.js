...
// tab — can fail, and if the book is only
...
// mailbox yet is recoverable. A mailbox with no tab is not, because Orca has
// no way to delete a Run — so the tab is always the one made and written down
// first, and a run that never opened one asks for no mailbox either. Since
// #317 the run asks for none at all: the session's tab makes its own, in the
// step its launch line starts with, so a mailbox Orca will not make is no
// longer a way for the run to die (mailbox-attestation.test.js).
...
test('no second harness is started in the tab the first run made', async (t) => {
  ...
  assertCleanFailure(await box.run(['init', '--bots', 'bots', '--harness', 'claude']));
  const orphan = (await box.orca.terminals())[0];
  assert.deepEqual(typedInto(orphan).map(tokenless), [bareLaunch(box, 'claude', 'bot-father', 'daily')], 'the first run did type it in');
  ...
  assert.deepEqual(typedInto(inBook[0]).map(tokenless), [bareLaunch(box, 'claude', 'bot-father', 'daily')], 'and the second run must not type it in again');
  assert.deepEqual(
