...
test('no second harness is started in the tab the first run made', async (t) => {
  ...
  assertCleanFailure(await box.run(['init', '--bots', 'bots', '--harness', 'claude']));
  const orphan = (await box.orca.terminals())[0];
  assert.deepEqual(typedInto(orphan), [bareLaunch(box, 'claude', 'bot-father', 'daily')], 'the first run did type it in');
  ...
  assert.deepEqual(typedInto(inBook[0]), [bareLaunch(box, 'claude', 'bot-father', 'daily')], 'and the second run must not type it in again');
  assert.deepEqual(
